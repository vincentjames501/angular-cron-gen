...
(function () {
    ...
    angular.module('angular-cron-gen', []).factory('cronGenService', function () {
        return {
            ...
            appendInt: function appendInt(number) {
                var value = '' + number;
                if (value.length > 1) {
    ...
    }]).directive('cronGen', ['$templateRequest', '$sce', '$compile', '$log', 'cronGenService', function ($templateRequest, $sce, $compile, $log, cronGenService) {
        return {
            scope: {
                ngModel: '=',
                ngDisabled: '=',
            ...
            link: function link($scope, elem, attrs, _ref) {
                ...
                var selectOptions = $scope.selectOptions = {
                    months: [].concat(_toConsumableArray(new Array(11).keys())).map(function (x) {
                        return x + 1;
                    }),
                    monthWeeks: ['#1', '#2', '#3', '#4', '#5', 'L'],
                    days: ['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'],
                    minutes: [].concat(_toConsumableArray(new Array(59).keys())).map(function (x) {
                        return x + 1;
                    }),
                    hours: [].concat(_toConsumableArray(new Array(23).keys())).map(function (x) {
                        return x + 1;
                    }),
                    monthDays: [].concat(_toConsumableArray(new Array(31).keys())).map(function (x) {
                        return x + 1;
                    }),
                    monthDaysWithLasts: ['1W'].concat(_toConsumableArray([].concat(_toConsumableArray(new Array(31).keys())).map(function (x) {
                        return '' + (x + 1);
                    })), ['LW', 'L'])
                };
                ...
                if (formCtrl && attrs.name) {
                    ngModelCtrl.$validators.testCronExpr = function (expression) {
                        return cronGenService.isValid(cronFormat, expression);
                    };
                }
                ...
                $scope.monthDayDisplay = function (monthDay) {
                    if (monthDay === 'L') {
                        return 'Last Day';
                    } else if (monthDay === 'LW') {
                        return 'Last Weekday';
